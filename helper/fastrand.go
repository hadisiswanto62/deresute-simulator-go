...
func Uint32(semiSeed int) uint32 {
	v := rngPool.Get()
	if v == nil {
		v = &RNG{}
	}
...
//
// It is safe calling this function from concurrent goroutines.
func Uint32n(maxN uint32, semiSeed int) uint32 {
	x := Uint32(semiSeed)
...
func (r *RNG) Uint32(semiSeed int) uint32 {
	semiSeed64 := int64(semiSeed)
	for r.x == 0 {
		r.x = getRandomUint32(semiSeed64)
	}
...
//
// It is unsafe to call this method from concurrent goroutines.
func (r *RNG) Uint32n(maxN uint32, semiSeed int) uint32 {
	x := r.Uint32(semiSeed)
	// See http://lemire.me/blog/2016/06/27/a-fast-alternative-to-the-modulo-reduction/
	return uint32((uint64(x) * uint64(maxN)) >> 32)
}

func getRandomUint32(semiSeed int64) uint32 {
	x := time.Now().UnixNano() + semiSeed*34421
