...
func TestCardFields(t *testing.T) {
	...
	if have, want := card.Rarity.ID, uint8(3); want != have {
	...
	if have, want := card.Rarity.MaxLevel, uint8(40); want != have {
	...
	if have, want := card.VisualMax, uint16(3330); want != have {
		t.Errorf("Error on fields VisualMax! want = %v have = %v", want, have)
	}
	if have, want := card.VisualMin, uint16(1965); want != have {
		t.Errorf("Error on fields VisualMin! want = %v have = %v", want, have)
	}
	if have, want := card.VocalMax, uint16(1728); want != have {
		t.Errorf("Error on fields VocalMax! want = %v have = %v", want, have)
	}
	if have, want := card.VocalMin, uint16(1020); want != have {
